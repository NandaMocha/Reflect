...
1. **Terms acknowledgment before first use** — `SpaceTermsSheet` is shown once before a
   ...
   controls.
2. **Report content** — every reflection and every response exposes a **Report** action
   (`ReportContentButton`) that opens a pre-filled email to the developer with the space
   name and the offending content's record ID.
...
4. **Member self-service** — any member can **leave** a space at any time; authors can
   **delete their own** reflections/responses.

## How to demo during review

1. Open the app → **Spaces** tab → accept the one-time terms sheet.
2. Create a space, add a reflection, add a response.
3. On any reflection/response, use the row's **Report…** action → a pre-filled report
   email opens.
